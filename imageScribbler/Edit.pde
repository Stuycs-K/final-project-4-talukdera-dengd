public class Edit{
  ...
  void writeRedPlane(PImage img, int plane, ArrayList<int[]> points){
    img.loadPixels();
    
    for (int i = 0; i < points.size(); i++) {
      int[] point = points.get(i);
      color pixel =img.pixels[Math.abs(point[0]*point[1])];
      int r = (int) red(pixel);
      int g = (int) green(pixel);
      int b = (int) blue(pixel);
      ...
      img.pixels[Math.abs(point[0]*point[1])] = color(r,g,b);
